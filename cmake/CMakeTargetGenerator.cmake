...
2) Input project file didn't specify any inputs either.]=])
    ...
    _doxygen_get(HTML_EXTRA_STYLESHEET _extra_stylesheet)
    if (_extra_stylesheet)
        list(APPEND _all_inputs "${_extra_stylesheet}")
    endif()
    _doxygen_get(HTML_EXTRA_FILES _extra_files)
    if (_extra_files)
        separate_arguments(_extra_files)
        list(APPEND _all_inputs ${_extra_files})
    endif()
    _doxygen_get(HTML_HEADER _header)
    if (_header)
        list(APPEND _all_inputs "${_header}")
    endif()
    _doxygen_get(HTML_FOOTER _footer)
    if (_footer)
        list(APPEND _all_inputs "${_footer}")
    endif()
    _doxygen_get(LAYOUT_FILE _layout_file)
    if (_layout_file)
        list(APPEND _all_inputs "${_layout_file}")
    endif()
