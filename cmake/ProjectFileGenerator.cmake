...
function(_doxygen_parse_input _out_var _property)
    global_map_get("option_args" _option_args)
    global_map_get("one_value_args" _one_value_args)
    global_map_get("multi_value_args" _multi_value_args)
...
function(_doxygen_property_add _path _type)
    global_map_get(doxygen.updatable.properties _properties)
    ...
    global_map_set(${_path}_TYPE ${_type})
    if (_type STREQUAL STRING)
        global_map_append(one_value_args ${_path})
        #global_map_set(${_path}_INPUT ${IN_INPUT_STRING})
    endif ()
    if (_type STREQUAL OPTION)
        global_map_append(option_args ${_path})
        #global_map_set(${_path}_INPUT ${IN_INPUT_OPTION})
    endif ()
    if (_type STREQUAL LIST)
        global_map_append(multi_value_args ${_path})
        #global_map_set(${_path}_INPUT "${IN_INPUT_LIST}")
    endif ()
    if (DEFINED IN_DEFAULT AND NOT ${_path}_DEFAULT IN_LIST _index)
        global_map_set(${_path}_DEFAULT ${IN_DEFAULT})
    endif ()
    if (DEFINED IN_SETTER)
        global_map_set(${_path}_SETTER ${IN_SETTER})
    endif ()
    if (DEFINED IN_UPDATER)
        global_map_set(${_path}_UPDATER ${IN_UPDATER})
    endif ()
    global_map_get(${_path}_OVERWRITE _prev_overwrite)
    if (_prev_overwrite STREQUAL "")
        global_map_set(${_path}_OVERWRITE ${IN_OVERWRITE})
    endif ()

    global_map_append(doxygen.updatable.properties "${_path}")
endfunction()
...
function(_doxygen_update_path _path)
    #global_map_get(${_path}_INPUT _input_param)
    global_map_get(${_path}_SETTER _setter)
    global_map_get(${_path}_UPDATER _updater)
    global_map_get(${_path}_DEFAULT _default)
    global_map_get(${_path}_TYPE _type)
    ...
    if (_input_value STREQUAL "")
        ...
        #if (_input_param)
        #    global_map_set(${_input_param} "${_value}")
        # endif ()
    else()
...
function(_doxygen_property_apply_setter _path _name _out_var)
    if (_name)
        global_map_get(${_path}_OVERWRITE _overwrite)
...
function(_doxygen_property_apply_default _property
    ...
    if (NOT _default STREQUAL "")
        global_map_get(${_property}_OVERWRITE _overwrite)
        _doxygen_log(DEBUG "[_doxygen_property_apply_default(${_property})] ${_property}_OVERWRITE = ${_overwrite}; _input_value = ${_input_value}")
